...
例1 数集 $\operatorname{Q}({\sqrt{2}})=\;|a+b{\sqrt{2}}\mid a,\;b\in\operatorname{Q}|$ 是一个数域.
...
$$
\begin{array}{r l}{
    \displaystyle f(x)\mid(u_{1}(x){g}_{1}(x)+u_{2}(x){\it g}_{2}(x)+\cdots+u_{r}(x){g}_{r}(x)}\end{array}
$$
...
再证唯一性.设有另外 $p(x)\,,\;s(x)\in\operatorname{F}[x]$,使得
...
*推论 设 $f(x),g(x) \in \mathbb{F[x]}$,则 $g(x)$ 整除 $f(x)$的充分必要素件是用 $g(x)$ 除 $f(x)$,余式为零*
...
易见多项式的最大公因式不是唯一的。$f(x)$ 和 $g(x)$ 的首项系数为1的最大公因式记作 $(f(x),g(x))$.根据定义1.4-1 中的条件(2)和定理1.3-1,
$f(x)$,$g(x)$ 的最大公因式只差非零常数倍.
