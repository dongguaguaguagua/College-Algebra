...
我们常见的数域有复数域、实数域和有理数域. 它们分别记为 $\CC$, $\RR$ 和 $\QQ$.易证下面两个不常见的例子是数域.
...
定义 1.4-2 设 $f(x),g(x) \in \FF[x]$ ,若 $(f(x),g(x)) = 1$, 则称 $f(x)$ 与 $g(x)$ **互素**.

定理 1.4-4 设 $f(x),g(x) \in \FF[x]$ ,则 $f(x)$ 与 $g(x)$ 互素的充分必要条件是存在多项式 $u(x)$ 和 $v(x)$ ,使得
$$
1=u(x)f(x)+v(x)g(x).
$$
证明 必要性由定理 1.4-2 立即可得. 现证明充分性. 假设 $1=u(x)f(x)+v(x)g(x)$ ,且 $d(x)=(f(x),g(x))$ ,则 $d(x)|f(x),d(x)|g(x)$,从而
$$
...
$$
由定理 1.4-1 得 (f_1(x),g_1(x))=1.

推论4 设 $(f(x),g(x))=d(x)$ 且 $h(x)$ 是首项系数为1的多项式,则
$$
